...
namespace Listak
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (patron.StartsWith("Canon"))
                    { counter++; }
            }
            Console.WriteLine($"{counter}db Canon gyártmányú patron van a listában");
            //10
            bool plusSign = false;
            for (int i = 0; i < jelek.Count; i++)
            {
                ...
                if ((jelek[i] == '+') && plusSign)
                {
                    Console.WriteLine($"A c# indexelése szerint a az első ++ elem a listában az:{i}");
                    break;
                }else { plusSign = false; }
        ...
        {
            Bogobogo(list, list.Count);
        }
        private static void Bogobogo(List<int> list, int n)
        {
            if (n <= 1)
                return;
        ...
        private static bool IsSorted(List<int> list, int n)
        {
            for (int i = 1; i < n; i++)
                if (list[i] < list[i - 1])
                    return false;
